 ...
 *
 * Server component with centered card. "Sign in with Google" links to
 * /api/auth/google which initiates the OAuth flow server-side.
 */

type SearchParams = Promise<{ error?: string }>;

const ERROR_MESSAGES: Record<string, string> = {
  domain_not_allowed: "Your email domain is not registered with this organization.",
...
export default async function LoginPage(props: { searchParams: SearchParams }) {
  ...
  const errorMessage = error ? ERROR_MESSAGES[error] ?? "An unknown error occurred." : null;
    ...
    <div className="flex min-h-screen items-center justify-center bg-gray-50">
      <div className="w-full max-w-md rounded-xl bg-white p-8 shadow-lg">
        ...
        <div className="mb-8 text-center">
          <h1 className="text-3xl font-bold text-gray-900">DriveWiki</h1>
        ...
        <p className="mt-6 text-center text-xs text-gray-400">
          Only authorized Google Workspace accounts can access this system.
        </p>
      </div>
    </div>
  );
